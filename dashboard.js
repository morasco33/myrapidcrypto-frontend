...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function loadActiveInvestments() { 
    ...
    try {
        ...
        document.querySelectorAll('.withdraw-btn-plan').forEach(btn => {
          btn.addEventListener('click', async function handleWithdrawClick() {
            const investmentId = btn.dataset.investmentId;
            const withdrawalPin = prompt("Enter your 5-digit withdrawal PIN:");
            if (!withdrawalPin || !/^[0-9]{5}$/.test(withdrawalPin)) {
              alert("Invalid PIN format. Please enter a 5-digit number.");
              return;
            }

            showModal('Processing Withdrawal...', 'Please wait...');
            try {
              const res = await fetch(`${DASH_API_BASE_URL}/investments/${investmentId}/withdraw`, {
                method: 'POST',
  ...
  document.querySelectorAll('.invest-btn').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      const amountStr = prompt(`Enter investment amount ($${minAmount} - $${maxAmount}):`);
      if (!amountStr) return;

      const amount = parseFloat(amountStr);
      if (isNaN(amount) || amount < minAmount || amount > maxAmount) {
        alert(`Invalid amount. Enter between $${minAmount} and $${maxAmount}.`);
        return;
      }
      ...
      try {
        ...
        if (!res.ok || !data.success) throw new Error(data.message || 'Investment failed');
